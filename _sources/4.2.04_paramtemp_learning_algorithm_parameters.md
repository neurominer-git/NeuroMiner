...
In addition to standard loss functions, NeuroMiner supports optimization using performance-criterion–based soft losses, enabling the training objective to directly reflect the evaluation metric of interest. When the option loss = 'performance_criterion' is selected, the optimization loss is automatically adapted to match the chosen NeuroMiner performance criterion, ensuring consistency between model optimization and TensorFlow parameter updates.

For classification tasks, the following performance criteria are available:
Accuracy (ACCURACY), True Positive Rate (TPR), False Positive Rate (FPR), Positive Predictive Value (PPV), Matthews Correlation Coefficient (MCC), Area Under the ROC Curve (AUC), Geometric Mean (GMEAN), Balanced Accuracy (BAC), Enhanced Balanced Accuracy (BAC2), F-score (FSCORE), Prognostic Summary Index (PSI), Number Needed to Predict (NNP), Positive Likelihood Ratio (PLR), and Negative Likelihood Ratio (NLR). These metrics are implemented as differentiable surrogate loss functions, allowing direct optimization of clinically and statistically meaningful classification performance measures.

For regression tasks, the supported performance criteria include Mean Squared Error (MSE), Normalized Root Mean Squared Deviation (NRMSD), Squared Correlation Coefficient (SCC), Pearson Correlation Coefficient (CC), Mean Absolute Error (MAERR), and Inverse-Density Weighted Mean Squared Error (MSEINVDENS). These losses are fully differentiable and enable optimization toward scale-sensitive, correlation-based, or distribution-aware regression objectives.

This design allows NeuroMiner to align model training with the specific performance criterion used for model selection, evaluation, or clinical interpretation.
...
For more guidance on how to define models with files, see these [**Example files**](https://github.com/neurominer-git/NeuroMiner/tree/main/Python/example_python_files).
