...
$$
\mathrm{Var}(w_{\mathrm{agg}})=\sum_c w_c^2 \mathrm{Var}(v_c) + 2\!\!\sum_{1\leq c<d\leq C} \! w_c w_d\, \mathrm{Cov}(v_c, v_d).
$$
...
- **Threshold‑matched**: determine observed cutoff $t_{\mathrm{obs}}$; select all $c$ with $S^{(p)}_c \geq t_{\mathrm{obs}}$ (trim/pad to $K$ if needed).

4) For feature $f$ in selected slot $c$, compute empirical p:
$$
\hat p_{f,c} = \frac{E_{f,c}+1}{|P_f|+1},\quad E_{f,c}=\sum_{p\in P_f}\mathbf{1}\!\left(\bigl|w^{(p)}_{f,c}\bigr| \geq \bigl|w_{f,c}\bigr|\right).
$$
...
$$
L^{(m)}_p=\frac{\sum_{c\in\mathcal{C}}\bigl\|w^{(m)}_c\bigr\|_p}{\sum_{m^{\prime}}\sum_{c\in\mathcal{C}}\bigl\|w^{(m^{\prime})}_c\bigr\|_p}
$$
- **Component×modality share** (factor‑resolved), for $c\in\mathcal{C}_{\mathrm{sig}}$:
$$
L^{(m)}_{p,c}=\frac{\bigl\|w^{(m)}_c\bigr\|_p}{\sum_{m^{\prime}}\bigl\|w^{(m^{\prime})}_c\bigr\|_p}.
$$
