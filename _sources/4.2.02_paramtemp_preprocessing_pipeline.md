...
**5 | Spatial correlations with molecular and cellular brain maps**

This functionality allows users to combine brain imaging data with molecular and cellular brain maps from a healthy volunteer population in order to link neuroimaging patterns with underlying neurobiological mechanisms. We adapted the JuSpace toolbox (https://github.com/juryxy/JuSpace) to compute cross-modal correlations between the imaging data and a set of user-selected brain maps. Thus the imaging data of of each participant is mapped to a vector for correlation coefficients that express the associations between the respective imaging metric and the molecular or cellular brain maps. See more at Spatial correlations with molecular and cellular brain maps below.
...
```{figure} Images/NM_preproc_brainmaps.png
---
...
### Handling unseen covariate groups

If new data contains covariate groups not observed during training, DIR handles them as follows:  
- **Global pooled distribution:** Unseen groups are corrected using the median or mean distribution pooled across all covariate groups seen during training. This ensures that all data points, including unseen groups, are adjusted toward the overall “ideal” distribution.  
...
>                                   
> 3 | MANHATTAN distance-based nearest-neighbor search (Scale / Standardize first!)
> 
> 4 | EUCLIDEAN distance-based nearest-neighbor search (Scale / Standardize first!)
>
> 5 | Sequential kNN-based imputation method (EUCLIDEAN)       
>                     
> 6 | Downshifted Gaussian imputation (for left-censored data)  
>                    
> 7 | SEUCLIDEAN distance-based nearest-neighbor search   
>                         
> 8 | COSINE similarity-based nearest-neighbor search   
>                           
> 9 | HAMMING distance-based nearest-neighbor search 
>                              
>10 | JACCARD distance-based nearest-neighbor search   
>                            
> 11 | Downshifted Gaussian imputation (for left-censored data)      
>               
> 12 | Nearest-neighbor imputation using hybrid method
...
Use this method only when missing values are Missing Not At Random (MNAR) and reflect low signal, such as in proteomics where low-abundance features often fall below the detection limit. For each feature independently, the method estimates the mean and standard deviation from the observed values, then replaces missing entries by random samples drawn from a Gaussian that is shifted toward lower intensities and scaled down in variance.
...
---
## Spatial correlations with molecular and cellular brain maps

This function calculates whole-brain correlations between sample-dependent imaging (e.g., structural or functional imgaging) and molecular (e.g., neurotransmitter average density) and cellular brain maps from a healthy volunteer population. An atlas is used to derive mean values per atlas region in both modalities and these values are use for the calculation of the correlation coefficients. The functionality was adapted from the JuSpace toolbox: https://github.com/juryxy/JuSpace. For detailed information about the methods, please refer to the JuSpace publication (see below).

There is an option to download atlases and brain maps. Please cite the respective software in your publication:
...
This function has been updated to allow feature-ranking within the preprocessing pipeline. As this modification is not backward-compatible with NeuroMiner 1.3, users are required to re-import atlases and brain maps for analyses conducted in NeuroMiner 1.4 and subsequent versions.

```{figure} Images/BrainMapsSetup.png
---
...
**2 | Download brain maps from JuSpace or neuromaps**

Indicate whether you would like to download or use existing brain maps from the JuSpace (MATLAB-based) or neuromaps (Python-based) toolbox. Download from neuromaps will only be available if Python is configured in MATLAB. Select a directory for saving the brain maps or provide a path to already downloaded maps. It is also possible to use your own brain maps. For this, you can select your own brain maps directory in step 6.

**3 | Atlas selection**

Select an atlas file (*.nii) containing the regions-of-interest for the calculation of mean values.

**4 | Alternative name(s) for atlas(es)**

Provide an alternative (e.g., shorter) name for the selected atlas. The default will use the atlas filename without the file extension (e.g., for feature naming in the visualization module). This step will only become available if an atlas was selected.

**5 | Correlation type**

Choose between Spearman (recommended) or Pearson correlation coefficients.

**6 | Adjust for spatial autocorrelations**

Choose between correcting for spatial autocorrelations (recommended) or not.

**7 | Brain maps directory**

Select the brain maps directory containing *.nii brain maps. Note: this

**8 | Brain maps selection**

Choose the brain maps you would like to use in your analysis. This step will only become available if a brain maps directory has been selected.

**9 | Alternative names for brain maps**

Provide an alternative (e.g., shorter) name for the selected brain maps. The default will use the brain maps filenames without the file extension (e.g., for feature naming in the visualization module). This step will only become available if brain maps were selected.

**10 | Import brain maps and atlas(es)**

Import brain map and atlas information. This step is important to complete the setup and will only become available if steps 3 to 9 are defined.
