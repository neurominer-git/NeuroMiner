function imputed_data = lp_DownshiftGaussImp(data, shift_factor, scale_factor, RNG_STREAM)
...
% - If observed SD <= eps or non-finite, falls back to constant fill at mu_shifted.
    ...
    for j = 1:nFeatures
        ...
        if sigma_impute <= eps || ~isfinite(sigma_impute)
            ...
            imputed_vals = mu_impute + sigma_impute * randn(RNG_STREAM, sum(missing_idx), 1);
        end
