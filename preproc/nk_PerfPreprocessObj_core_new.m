% =========================================================================
% FORMAT [SrcParam, InputParam, TrParam] = nk_PerfPreprocessObj_core( ...
%                                   SrcParam, InputParam, TrParam, act)
% =========================================================================
%
% OVERVIEW
%   Master pre-processing pipeline for NeuroMiner. Executes a user-defined
%   sequence of actions (act_*) on training, test, and optional calibration
%   data; learns and/or applies step-wise preprocessing parameters; and
%   manages cross-validation (CV) vs. out-of-sample (OOCV) execution.
%
%   When hyperparameters are present at a step, the function expands data
%   into parameter “shelves” (cells) and returns both the tree of processed
%   data and the corresponding learned parameters for each shelf.
%
% INPUTS
%   SrcParam    struct  Source-level info (labels, covariates, flags, etc.).
%                       Also used to store synthetic-data expansions.
%
%   InputParam  struct  Containers and settings for data & steps:
%                       .Tr              training data  (matrix or cell shelves)
%                       .Ts              test data      (matrix, cell, or {cell,k} for multiple sets)
%                       .C               calibration data (optional; matrix or cell)
%                       .P{1:n}          per-step configs incl. subfields per action
%                       .CV1perm, .CV1fold (CV bookkeeping)
%                       .curclass        class index for multi-label problems
%                       .Yw              per-shelf weights/external vectors (optional)
%
%   TrParam     [] | cell | struct
%                       - Empty   => CV/training mode: learn parameters at each step.
%                       - Nonempty=> OOCV mode: apply provided parameters; no re-learning.
%
%   act         char | cellstr
%                       Action name (e.g., 'scale') or cell array of step names
%                       in execution order: {'impute','standardize','reducedim',...}
%
% OUTPUTS
%   SrcParam    struct  Possibly augmented (e.g., synthetic label/covariate shelves).
%   InputParam  struct  Data after all steps. On hyperparam sweeps / shelves:
%                       .Tr, .Ts, .C, .Yw become cell arrays aligned with TrParam.
%   TrParam     cell    Per-step learned parameters (training mode), or the input
%                       parameters passed through (OOCV mode). Matches the step layout
%                       and data shelves (cells indexed by [param-combination, label]).
%
% MODES & BEHAVIOR
%   • Training (CV) mode     : TrParam is empty → learn step parameters on .Tr; apply to .Ts/.C.
%   • Out-of-sample (OOCV)   : TrParam provided → apply parameters to .Ts/.C (no re-learning).
%   • Multi-label handling   : If preprocessing depends on labels, separate shelves per label;
%                              otherwise reuse single processed data to save memory/time.
%   • Hyperparameter sweeps  : For steps with .opt grids, expand shelves over all combinations.
%   • Synthetic data         : If enabled (global SYNTH or SVM.ADASYN), training data/labels/
%                              covariates are expanded at eligible steps.
%   • Template parameters    : If provided via global TEMPL, some steps can reuse template mpp, etc.
%   • Efficient copy for Ts  : If Ts{1} equals Tr at entry, uses an optimized path (copy_ts).
%
% GLOBALS (read)
%   VERBOSE, TEMPL, SVM, MODEFL, SYNTH
%
% KEY ACTIONS (act_*)
%   impute            : nk_PerfImputeObj           – feature-wise imputation
%   labelimpute       : nk_PerfImputeLabelObj      – label imputation (where supported)
%   scale             : nk_PerfScaleObj            – min–max to [0,1] or [-1,1]
%   normalize         : nk_PerfNormObj             – group-mean centering (optional indices)
%   unitnormalize     : nk_PerfUnitNormObj         – L2 norm to unit length
%   remmeandiff       : nk_PerfRemMeanDiffObj      – remove group offsets from global means
%   discretize        : nk_PerfDiscretizeObj       – binning to specified edges
%   symbolize         : nk_PerfDiscretizeObj       – symbolization via discretization
%   standardize       : nk_PerfStandardizeObj      – z-scoring (winsorization optional)
%   reducedim         : nk_PerfRedObj              – DR (e.g., PCA/ICA/PLS; Procrustes optional)
%   extdim            : nk_PerfExtDimObj           – extract subspaces from reduced data
%   correctnuis       : nk_UnivCorrProcsObj        – partial corr / ComBat / Disparate Impact Remover
%   elimzero          : nk_PerfElimZeroObj         – prune zero/near-constant features (+mask repair)
%   rankfeat          : nk_PerfFeatRankObj         – univariate/multivariate feature ranking
%   extfeat           : nk_PerfWActObj             – select/aggregate features by learned weights
%   remvarcomp        : nk_PerfAdjForCovarsUsingPCAObj – remove variance components via PCA
%   devmap            : nk_PerfDevMapObj           – deviation-based feature weighting (SPLS params)
%   graphComputation  : cv_graph_PerfGraphConstruction – construct graphs (normative, etc.)
%   graphSparsity     : cv_graph_PerfSparsityThres – threshold/sparsify graphs
%   graphMetrics      : cv_graph_PerfGraphMetrics  – compute graph metrics
%   JuSpace           : cv_perfJuSpace             – neurotransmitter correlation features
%   ROImeans          : cv_perfROImeans            – region-of-interest aggregation
%   skewcorr          : nk_PerfSkewCorrObj         – skewness correction (log/Box–Cox/Yeo–Johnson)
%   customPreproc     : cv_perfCustomPreproc       – user-supplied preprocessing hook
%
% NOTES
%   • Action configs live in InputParam.P{i}.<ACTION>, with per-step bookkeeping in TrParam.
%   • When shelves are created, indices are: [parameter-combination, label].
%   • For calibration data .C (if present) some actions mirror the .Ts processing.
%   • Set VERBOSE to print per-step timing and details.
%   • Debug: set local writeout=true to persist intermediate Out structures per step/shelf.
%
% EXAMPLE
%   act  = {'impute','standardize','reducedim','extfeat'};
%   TrParam = []; % training mode
%   [SrcParam, InputParam, TrParam] = nk_PerfPreprocessObj_core( ...
%       SrcParam, InputParam, TrParam, act);
%
% =========================================================================
%   (c) Nikolaos Koutsouleris and contributors, 01/2024–2025
% =========================================================================
    ...
    elseif isfield(InputParam.P{i},'opt') && ~isempty(InputParam.P{i}.opt)
        ...
        for nl = 1:nL
            ...
            for l = 1: nO
                ...
                % Take care of synthetic labels and covars expansions if needed.
                if synthfl
...
switch IN.METHOD
    ...
    case 2
        % method 2: COMBAT HARMONIZATION (back/forward compatible).
        % ----------------------------------------------------------
        % Legacy prints
        if VERBOSE
            fprintf('\tComBat harmonization ...');
            if isfield(InputParam.P{i},'MCOVARUSE') && InputParam.P{i}.MCOVARUSE == 1
                % TrMod is built later in the pipeline; keep legacy message
                if isfield(InputParam.P{i},'TrMod')
                    fprintf(' retaining variance from %d variables...', size(InputParam.P{i}.TrMod, 2));
                else
                    fprintf(' retaining variance from selected covariates...');
                end
            end
            if isfield(InputParam.P{i},'MCOVARLABEL') && InputParam.P{i}.MCOVARLABEL == 1
                fprintf(' including label...');
            end
        end
        ...
        % -------- New settings with safe fallbacks --------
        % Enable/disable batch (default ON to match legacy)
        if isfield(InputParam.P{i},'ENABLE_BATCH')
            IN.enable_batch = logical(InputParam.P{i}.ENABLE_BATCH);
        elseif isfield(InputParam.P{i},'BATCH_ON')
            IN.enable_batch = (InputParam.P{i}.BATCH_ON == 1);
        else
            IN.enable_batch = true; % legacy default: batch ON
        end

        % Options for the new combat() (keep/remove, spline, reference_level, CovBat)
        if isfield(InputParam.P{i},'COMBAT_OPTS') && ~isempty(InputParam.P{i}.COMBAT_OPTS)
            IN.combat_opts = InputParam.P{i}.COMBAT_OPTS;
        else
            % Build minimal opts from legacy fields so old projects still behave
            IN.combat_opts = struct();
            if isfield(InputParam.P{i},'REFERENCE_LEVEL') && ~isempty(InputParam.P{i}.REFERENCE_LEVEL)
                IN.combat_opts.reference_level = InputParam.P{i}.REFERENCE_LEVEL;
            end
            if isfield(InputParam.P{i},'MCOVARUSE') && InputParam.P{i}.MCOVARUSE == 1 ...
                    && isfield(InputParam.P{i},'MCOVAR') && ~isempty(InputParam.P{i}.MCOVAR)
                % Legacy "retain variance" => keep_idx in original coordinates
                IN.combat_opts.keep_idx = InputParam.P{i}.MCOVAR;
            end
            if isfield(InputParam.P{i},'COVBAT_MODE') && InputParam.P{i}.COVBAT_MODE == 2
                IN.combat_opts.useCovBat = true;
                if isfield(InputParam.P{i},'COVBAT_K') && ~isempty(InputParam.P{i}.COVBAT_K)
                    IN.combat_opts.K = InputParam.P{i}.COVBAT_K;
                end
                if isfield(InputParam.P{i},'COVBAT_VAR') && ~isempty(InputParam.P{i}.COVBAT_VAR)
                    IN.combat_opts.var_expl = InputParam.P{i}.COVBAT_VAR;
                end
            end
        end

        % A tiny, user-facing note when batch is OFF
        if VERBOSE && ~IN.enable_batch
            fprintf(' [batch OFF → no-batch residualization]');
        end
...
if paramfl && tsfl && isfield(TrParami,'beta') 
    ...
    tsproc = true;
elseif trfl
    % Deal with synthetic data appropriately
    if actparam.synthfl
        % Append synthetic covariates/labels as before, but only touch batch covariate
        % if ComBat is in use AND batch is enabled. We must not index (:, IN.COVAR)
        % when IN.enable_batch is false or COVAR missing (back-compat safe).
        if iscell(SrcParam.covarsSyn)
            % Always extend TrCovars with the full covariate row block (safe),
            % because downstream code may use additional covariates for mod / labels.
            IN.TrCovars = [ InputParam.P{i}.TrCovars; SrcParam.covarsSyn{actparam.j} ];
            % If ComBat, extend TrMod with synthetic labels when present (legacy behavior)
            if IN.METHOD == 2 && isfield(InputParam.P{i},'TrMod')
                IN.TrMod = [ InputParam.P{i}.TrMod; SrcParam.TrainLabelSyn{actparam.j}(:,actparam.curlabel) ];
            end
        else
            IN.TrCovars = [ InputParam.P{i}.TrCovars; SrcParam.covarsSyn ];
            if IN.METHOD == 2 && isfield(InputParam.P{i},'TrMod')
                IN.TrMod = [ InputParam.P{i}.TrMod; SrcParam.TrainLabelSyn(:,actparam.curlabel) ];
            end
        end
    end
    [InputParam.Tr, TrParami] = nk_UnivCorrProcsObj(InputParam.Tr, IN);
    if tsfl, tsproc = true; end
else
    if VERBOSE;fprintf('not performed'); end
end
...
if tsproc
    if copy_ts
        ...
        Ts = nk_UnivCorrProcsObj(InputParam.Ts(2:end), TrParami); 
        InputParam.Ts(2:end) = Ts;
    else
        InputParam.Ts = nk_UnivCorrProcsObj(InputParam.Ts, TrParami); 
    end
end 
...
if isfield(CALIB,'calibflag') && CALIB.calibflag && isfield(InputParam, 'C')
    ...
    %if isfield(InputParam, 'C') %CALIB.calibflag && CALIB.preprocstep > i
    InputParam.C = nk_UnivCorrProcsObj(InputParam.C, INcalib);
    %end
end
...
elseif trfl
    ...
    % Check for external weighting mask and adjust
    if isfield(InputParam,'Yw')
        if iscell(InputParam.Yw)
            for p = 1:numel(InputParam.Yw)
                for q=1:height(InputParam.Yw)
                    InputParam.Yw{p} = InputParam.Yw{p}(q,TrParami.NonPruneVec);
                end
            end
        else
            for q=1:height(InputParam.Yw)
                InputParam.Yw = InputParam.Yw(q,TrParami.NonPruneVec);
            end
        end
    end
    if tsfl, tsproc = true; end
...
% InputParam.P{i}.ind = TrParami.ind;
if ~isfield(actparam.RANK,'ind')
    actparam.RANK.ind = {};
    actparam.RANK.ind_opt = [];
end

actparam.RANK.ind_opt(end+1,1) = actparam.opt;
actparam.RANK.ind{end+1,1} = TrParami.ind;
...
elseif trfl
    ...
    if isfield(actparam,'RANK') && isfield(actparam.RANK,'ind')
        INDExtFeat = actparam.RANK.ind{actparam.j,1};
    else
        INDExtFeat = [];
    end
    [InputParam.Tr, TrParami] = cv_perfJuSpace(InputParam.Tr,InputParam.P{i}.JUSPACE,INDPruneVec,INDExtFeat);

    if tsfl, tsproc = true; end
end

if tsproc, InputParam.Ts = cv_perfJuSpace(InputParam.Ts, TrParami, INDPruneVec,INDExtFeat); end
end
% =========================================================================
...
% =========================================================================
function [SrcParam, InputParam, TrParami, actparam ] = act_skewcorr(SrcParam, InputParam, ~, TrParami, actparam)
% act_skewcorr: Perform Skewness Correction (log, Box–Cox, Yeo–Johnson) on 
%               training and test data. Called by nk_PerfPreprocessObj_core.
%
% INPUTS:
%   SrcParam : struct with global settings (labels, config)
%   InputParam : struct with fields .Tr (train data), .Ts (test data), .P, etc.
%   TrParam  : previously trained params from earlier steps (if needed)
%   TrParami : parameters for this step (either newly computed or stored from prior run)
%   actparam : struct with flags (trfl, tsfl, paramfl, etc.)
%
% OUTPUTS:
%   SrcParam  : possibly updated (e.g., if synthetic data, label adjustments)
%   InputParam: updated data (train/test) after skew correction
%   TrParami  : updated transformation parameters (e.g. offsets, lambdas)
%   actparam  : pass-through actparam

global VERBOSE CALIB

trfl    = actparam.trfl;    % true if we have training data
tsfl    = actparam.tsfl;    % true if we have test data
paramfl = actparam.paramfl; % true if out-of-sample parameters exist
i       = actparam.i;       % current preprocessing step index
...
if isfield(actparam,'opt') && ~isempty(actparam.opt)
    InputParam.P{i}.SKEWCORR.SkewThr = nk_ReturnParam('skewthr', InputParam.P{i}.Params_desc, actparam.opt); 
    InputParam.P{i}.SKEWCORR.BoxCoxLambdaVal = nk_ReturnParam('boxcoxlambda', InputParam.P{i}.Params_desc, actparam.opt); 
    InputParam.P{i}.SKEWCORR.YJLambdaVal = nk_ReturnParam('yjlambda', InputParam.P{i}.Params_desc, actparam.opt); 
end

% If we already have stored transformation parameters (TrParami) and are in 
% out-of-sample mode with test data => just apply to test data
if paramfl && tsfl && isfield(TrParami, 'SKEWCORR') 
    % We assume TrParami.SKEWCORR holds necessary info for applying transforms
    tsproc = true;
else
    % Otherwise, if we have training data => do "forward" transform on train
    if trfl
        [InputParam.Tr, TrParami] = nk_PerfSkewCorrObj(InputParam.Tr, InputParam.P{i}.SKEWCORR);
        % Suppose nk_PerfSkewCorrObj returns a second output "TrParami" 
        % that has offsets, lambdas, etc. in TrParami.SKEWCORR 
    end
...
if tsproc
    InputParam.Ts = nk_PerfSkewCorrObj(InputParam.Ts, TrParami);
end

% we might also handle calibration data (InputParam.C) if it exists, 
% or synthetic label data, etc. following the same pattern 
% (like in act_scale, act_standardize).
if CALIB.calibflag && isfield(InputParam, 'C') %&& CALIB.preprocstep > i 
    InputParam.C = nk_PerfSkewCorrObj(InputParam.C, InputParam.P{i}.SKEWCORR);
end
