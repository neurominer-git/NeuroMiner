...
figdata.hPanel      = uipanel('Units','norm', 'Position',hText.Extent, 'BorderType','line', 'BackgroundColor', [.6 .7 .6], 'Visible','off');
...
if AltAx
    ...
    xl      = get(gca,'Xlim');
    p       = polyfit(lxL(id1),predh(id1),1);     % p returns Ľ2 coefficients fitting r = a_1 * x + a_2
...
if isfield(handles.BinClass{h}.contingency,'AUC')
  flg='on';
  [handles.hroc, handles.hroc_random] = display_roc(handles, labelh, predh);
else
  flg = 'off';
end     
handles.axes1.XTickLabelMode = 'auto';
