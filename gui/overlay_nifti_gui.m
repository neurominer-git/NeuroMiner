...
% Collect your important handles (add all you create)
add = @(S,name,val) setfield(S, name, val); 
has = @(v) (exist(v,'var')==1) && ~isempty(eval(v)) && all(isgraphics(eval(v)));

handles = struct();

% Axes / colorbar
handles.hAx      = hAx;
handles.hCbarAx  = hCbar;
if exist('h3DAx','var'),       handles.h3DAx      = h3DAx;      end

% Region/struct controls
handles.hRegLabel = hRegLabel;
if has('hStructPopup'), handles.hStructPopup = hStructPopup; end
if has('hStructBtn'),   handles.hStructBtn   = hStructBtn;   end

% Color buttons near colorbar
handles.hNegBtn = hNegBtn;
handles.hMidBtn = hMidBtn;
handles.hPosBtn = hPosBtn;

% Multi-slice and grid controls
handles.hMulti       = hMulti;
handles.hRowLabel    = hRowLabel;
handles.hRowPop      = hRowPop;
handles.hColLabel    = hColLabel;
handles.hColPop      = hColPop;
handles.hStartLabel  = hStartLabel;
handles.hStartEdit   = hStartEdit;
handles.hEndLabel    = hEndLabel;
handles.hEndEdit     = hEndEdit;

% Viewmode group + buttons
handles.hViewGroup   = bg;
handles.hViewButtons = findobj(bg,'Type','uicontrol');  % Axial/Sagittal/Coronal/3D-View

% Slice slider + label
handles.hSliceLabel  = hSliceLabel;
handles.hSlice       = hSlice;

% 3D lighting/alpha controls
handles.hLightAzLbl  = hLightAzLbl;
handles.hLightAz     = hLightAz;
handles.hLightElLbl  = hLightElLbl;
handles.hLightEl     = hLightEl;
handles.h3DAlphaLbl  = h3DAlphaLbl;
handles.h3DAlpha     = h3DAlpha;

% 3D utility controls
handles.hBtnRecord   = hBtnRecord;
handles.hOverlayMode = hOverlayMode;
handles.hZoomIn      = hZoomIn;
handles.hZoomOut     = hZoomOut;
handles.hHome3D      = hHome3D;
handles.hSave3D      = hSave3D;
handles.hLoad3D      = hLoad3D;

% 3D sliders row (Azimuth/Elevation/Depth/BG/Ambient/Diffuse/Specular/SpecExp)
handles.hAzimTxt      = hAzimTxt;
handles.hAzimSlider   = hAzimSlider;
handles.hElevTxt      = hElevTxt;
handles.hElevSlider   = hElevSlider;
handles.hDepthTxt     = hDepthTxt;
handles.hDepthSlider  = hDepthSlider;
handles.hBGBtn        = hBGBtn;
handles.hAmbientTxt   = hAmbientTxt;
handles.hAmbientSlider= hAmbientSlider;
handles.hDiffuseTxt   = hDiffuseTxt;
handles.hDiffuseSlider= hDiffuseSlider;
handles.hSpecTxt      = hSpecTxt;
handles.hSpecSlider   = hSpecSlider;
handles.hSpecExpTxt   = hSpecExpTxt;
handles.hSpecExpSlider= hSpecExpSlider;

% Brightness/Alpha (bottom row)
handles.hBrightValue  = hBrightValue;
handles.hBrightSlider = hBrightSlider;
handles.hBrightLabel  = hBrightLabel;
handles.hAlphaValue   = hAlphaValue;
handles.hAlphaSlider  = hAlphaSlider;
handles.hAlphaLabel   = hAlphaLabel;

% Atlas controls (popup OR button)
if has('hAtlasPopup'), handles.hAtlasPopup = hAtlasPopup; end
if has('hAtlasBtn'),   handles.hAtlasBtn   = hAtlasBtn;   end

% ROI/overlay options (silhouette, crosshair, export, filter)
handles.hSilToggle    = hSilToggle;
handles.hSilColorTxt  = hSilColorTxt;
handles.hSilColor     = hSilColor;
handles.hCrossToggle  = hCrossToggle;
handles.hExportBtn    = hExportBtn;
handles.hFilterLbl    = hFilterLbl;
handles.hMinPct       = hMinPct;

% Contour controls
handles.hIsoToggle    = hIsoToggle;
handles.hIsoEdit      = hIsoEdit;

% Threshold controls (bi- and uni-polar)
handles.hNegLabel   = hNegLabel;
handles.hNegEdit    = hNegEdit;
handles.hNegUp      = hNegUp;
handles.hNegDn      = hNegDn;
handles.hNegSlider  = hNegSlider;

handles.hPosLabel   = hPosLabel;
handles.hPosEdit    = hPosEdit;
handles.hPosUp      = hPosUp;
handles.hPosDn      = hPosDn;
handles.hPosSlider  = hPosSlider;

handles.hThrLabel   = hThrLabel;
handles.hThrEdit    = hThrEdit;
handles.hThrUp      = hThrUp;
handles.hThrDn      = hThrDn;
handles.hThrSlider  = hThrSlider;

% A convenient flat list for bulk operations (reparent/units/visibility)
handles.all = [];
...
for kx = 1:numel(fn)
    ...
    if ~isempty(h)
        try
            h = h(:).';
            h = h(isgraphics(h));
            handles.all = [handles.all, h]; 
        catch
        end
    end
end
handles.all = unique(handles.all);
viewer = struct();
viewer.update = @updateStatsImage;
viewer.parent  = parent;
viewer.handles = handles;
viewer.reparent = @(newParent) nii_reparent(viewer, newParent);
viewer.layout   = @()           nii_layout(viewer);
viewer.enableAutoLayout = @()   nii_enableAutoLayout(viewer);
...
function nii_reparent(viewer, newParent)

    if ~isstruct(viewer) || ~isfield(viewer,'handles'), return; end

    % 1) Reparent top-level containers explicitly (axes, group)
    H = viewer.handles;

    safeSetParent(H, 'hAx',   newParent);
    safeSetParent(H, 'h3DAx', newParent);
    if isfield(H,'hViewGroup') && isgraphics(H.hViewGroup)
        set(H.hViewGroup, 'Parent', newParent);
    end
    % Colorbar: leave it attached to hAx; we'll position it, but don't reparent
    % if isa(H.hCbarAx,'matlab.graphics.illustration.ColorBar'), set(H.hCbarAx,'Location','manual'); end

    % 2) Reparent ONLY those controls whose parent is neither the group nor already under the panel
    toMove = [];
    for h = H.all
        if ~isgraphics(h) || ~isprop(h,'Parent'), continue; end
        if isa(h,'matlab.graphics.illustration.ColorBar'), continue; end  % skip colorbar
        par = get(h,'Parent');

        % If a child of the uibuttongroup, leave it (the group was reparented)
        if isgraphics(par) && strcmp(get(par,'Type'),'uibuttongroup')
            continue;
        end

        % If already under the panel (or a descendant of it), leave it
        if isDescendantOf(par, newParent)
            continue;
        end

        % Otherwise, mark to move
        toMove(end+1) = h; %#ok<AGROW>
    end
    if ~isempty(toMove), set(toMove, 'Parent', newParent); end

    % 3) Ensure normalized units ONLY for objects that are direct children of the panel
    %    (don’t touch units of controls that live inside other containers)
    try
        panelKids = findall(newParent, '-depth', 1);
        panelPix  = getpixelposition(newParent);
        for h = panelKids(:).'
            if ~isgraphics(h) || ~isprop(h,'Units'), continue; end
            u = get(h,'Units');
            if strcmpi(u,'pixels')
                posPx = get(h,'Position');
                posN  = [posPx(1)/panelPix(3), posPx(2)/panelPix(4), ...
                         posPx(3)/panelPix(3), posPx(4)/panelPix(4)];
                set(h, 'Units','normalized', 'Position', posN);
            end
        end
    catch
    end

    % 4) Let normalized children auto-resize with the panel
    try set(newParent,'AutoResizeChildren','on'); catch, end
end

function tf = isDescendantOf(childParent, ancestorPanel)
    tf = false;
    try
        h = childParent;
        while isgraphics(h) && isprop(h,'Parent')
            if isequal(h, ancestorPanel), tf = true; return; end
            h = get(h,'Parent');
...
function safeSetParent(H, field, newParent)
    if isfield(H, field)
        h = H.(field);
...
function nii_enableAutoLayout(viewer)
    par = viewer.parent;
    if ~isgraphics(par), return; end
    set(par,'SizeChangedFcn', @(~,~) nii_layout(viewer));
    nii_layout(viewer);  % initial placement
end

function nii_layout(viewer)
    H = viewer.handles;
    par = viewer.parent;
    if ~isgraphics(par), return; end

    % Panel size (pixels)
    p = getpixelposition(par);
    W = max(1,p(3)); Hgt = max(1,p(4));

    % Simple layout metrics
    m   = 12;      % outer margin
    cbW = 46;      % colorbar width
    axX = m; axY = m + 110;                    % leave a control strip at bottom
    axW = max(1, W - cbW - 3*m);
    axH = max(1, Hgt - axY - m);

    % Axes
    if isgraphics(H.hAx,'axes')
        set(H.hAx,'Units','pixels','Position',[axX axY axW axH]);
    end

    % Colorbar (manual location)
    if isa(H.hCbarAx,'matlab.graphics.illustration.ColorBar')
        set(H.hCbarAx,'Location','manual','Units','pixels', ...
            'Position',[axX+axW+m, axY, cbW, axH]);
    end

    % Example: center 3D sliders above axes
    cx = axX + axW/2; w = 160; h = 22; gap = 6; yTop = axY + axH - h - 8;
    trySet(H.hLightAz, 'Units','pixels','Position',[cx-w/2, yTop, w, h]);
    trySet(H.hLightEl, 'Units','pixels','Position',[cx-w/2, yTop-(h+gap), w, h]);
    trySet(H.h3DAlpha, 'Units','pixels','Position',[cx-w/2, yTop-2*(h+gap), w, h]);

    % Bottom strip examples (adjust to your handles)
    % trySet(H.hThresh, 'Units','pixels','Position',[axX, 12, 260, 22]);
    % trySet(H.hAlpha,  'Units','pixels','Position',[axX+270, 12, 260, 22]);
end

function trySet(h, varargin)
    if ~isempty(h) && all(isgraphics(h))
        try, set(h, varargin{:}); catch, end
    end
end
