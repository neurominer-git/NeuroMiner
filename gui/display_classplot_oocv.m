...
if handles.tglPercRank.Value
    ...
    if handles.tglP.Value
        if ~isfield(handles.OOCV(handles.oocvind).data.BinResults{h},'Calibration') || ...
                isempty(handles.OOCV(handles.oocvind).data.BinResults{h}.Calibration)
            P_oocv_h = probtransform(handles.BinClass{h}.labelh,P_h,P_oocv_h,'platt');
            P_h = probtransform(handles.BinClass{h}.labelh,P_h,[],'platt');
        else
            P_oocv_h = handles.OOCV(handles.oocvind).data.BinResults{h}.Calibration.Probabilities; 
            P_h = handles.BinClass{h}.Calibration.Probabilities; 
        end
        zeroln = 0.5;
    end
end
...
violin(P_h(id1),'x',1,'facecolor',handles.colptin(handles.BinClass{h}.groupind(g1),:), ...
    'edgecolor','none', 'facealpha', 0.1, 'bw', srth(P_h(id1)));% 'mc','','medc','');
...
violin(P_h(id2),'x',3,'facecolor',handles.colptin(handles.BinClass{h}.groupind(g2),:),...
    'edgecolor','none', 'facealpha', 0.1, 'bw', srth(P_h(id2)));% 'mc','','medc','');
...
if labels_known
    ...
    if (oocvfl1 && oocvfl2) 
     ...
     if sum(id1_oocv)
        violin(tP_oocv_h(id1_oocv),'x',kpos1 ,'facecolor',handles.colptin(handles.BinClass{h}.groupind(g1),:), ...
            'edgecolor','none', 'facealpha', 0.3, 'bw', srth(tP_oocv_h(id1_oocv)));
    ...
    if sum(id2_oocv)
        ...
        violin(tP_oocv_h(id2_oocv),'x',kpos2 ,'facecolor',CLR, ...
            'edgecolor','none', 'facealpha', 0.3, 'bw', srth(tP_oocv_h(id2_oocv)));% 'mc','','medc','');
    ...
    if sum(~ind0)
        violin(P_oocv_h(~ind0),'x',5,'facecolor','k', ...
            'edgecolor','none', 'facealpha', 0.3, 'bw', srth(P_oocv_h(~ind0)));% 'mc','','medc','');
    ...
    violin(P_oocv_h,'x',2,'facecolor','k',...
        'edgecolor','none', 'facealpha', 0.3, 'bw', srth(P_oocv_h));% 'mc','','medc','');
    ...
    case 1
        ...
        if sum(ind0)>2 %&& numel(unique(label_oocv_h(ind0)))>1 
            ...
            if isfield(contigmat,'AUC')
                flg2 = 'on'; [handles.hroc, handles.hroc_random] = display_roc(handles, label_oocv_h (ind0 & SubI), tP_oocv_h, handles.BinClass{h}.labelh, P_h);
            end
            %% Display pie charts
...
function bw = srth(x)
    ...
    bw = 0.9*min(sig, iqr_/1.34)*n^(-1/5);
