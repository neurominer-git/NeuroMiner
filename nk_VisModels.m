...
for i = 1 : nM
    ...
    if ~isfield(inp,'decompfl')
        ...
        if numel(inp.decompfl) == nM
            decompfl(i) = inp.decompfl(i);
        else
            if nM>1
                try
                    decompfl(i) = inp.decompfl(inp.tF(i));
                catch
                    decompfl(i) = inp.decompfl;
                end
            else
                try
                    decompfl(i) = inp.decompfl;
        ...
        switch inp.analmode 
            ...
            case 0
                ...
                if exist(oVISpath,'file') && ~ovrwrt && ~batchflag
                    ...
                    WriteCV2Data(inp, nM, MODEFL, FUSION, SAV, operm, ofold, I1);
                    if filefound
                        ...
                        for h=1:nclass % Loop through binary comparisons
                            ...
                            for m = 1 : nP % parameter combinations
                                ...
                                if any(permfl)
                                    ...
                                    if pmode(1)
                                        indpermA = nk_GenPermMatrix(CV, inp, nperms);
                                        if inp.nclass > 1
                                ...
                                for u=1:ul
                                    ...
                                    if ~any(permfl)
                                        ...
                                        for n=1:nM
                                            if width(Tx{n})>1
                                                Tx_perm{n} = zeros(size(Tx{n},1), size(Tx{n},2), nperms(n)); 
                                            else
...
if ~batchflag
    ...
    for n=1:nM
        ...
        for h=1:nclass
            ...
            %     case 'regression'
            %         [I.VCV2signconst_p{h,n}, I.VCV2signconst_pfdr{h,n}, I.VCV2signconst_z{h,n}, I.VCV2signconst{h,n}] = nk_SignBasedConsistencySignificance_SVR(I.VCV2VCV1{h,n}, 1000);  
            % end
...
function WriteCV2Data(inp, nM, MODEFL, FUSION, SAV, operm, ofold, I1)
    ...
    for h=1:inp.nclass
        for n=1:nM
            ...
            if datatype ==1 || datatype==2
                ...
                %     case 'regression'
                %         [~, SIGNCONST_FDR, ~, ~] = nk_SignBasedConsistencySignificance_SVR(I1.VCV1{h,n}, 1000);  
                % end
