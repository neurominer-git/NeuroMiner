...
% NM_TRAIN Executes the machine learning training and cross-validation in NeuroMiner
%
% This function configures and executes the ML pipeline in NeuroMiner for a specified
% analysis index. It supports interactive and batch modes, optional overwriting of
% existing results, and integration of precomputed data.
%
% INPUTS:
%   NM      - NeuroMiner structure containing the dataset and analysis setup.
%   analind - Index of the analysis to be trained.
%   inp     - (Optional) Input structure configuring run-time options, such as:
%             * lfl: Load-from-level flag (1: scratch, 2: preproc, 3: datamats, 4: results)
%             * preprocmat / gdmat / gdanalmat: Preloaded input matrices
%             * varstr: Variable suffix for naming
%             * concatfl: Concatenation flag for features
%             * ovrwrt: Overwrite flag (1 = overwrite, 2 = do not overwrite)
%             * update: Logical, whether to update existing results
%             * HideGridAct: Hide CV2 partition selection in GUI
%             * GridAct: Logical matrix [CV2x1 x CV2x2], indicating CV partitions to run
%             * batchflag: Run in batch mode (no GUI interaction)
%             * simFlag: If true, run simulation mode
%             * F: Modality indices to be used in fusion mode
%             * fixedOrder: Flag to indicate fixed feature order (relevant for stacking)
%             * multiflag: Whether multi-group classification is active
%             * nF: Number of modalities in fusion model
%             * curlabel: Active label index (for multilabel)
%             * id: ID string for output naming
%             * rootdir: Analysis output directory
%             * saveoptdir: Directory for saving optimal models
%             * lowmem: If true, disables computation-heavy features
%             * writeCV2: Whether to write CV2 weight maps to disk
%
% OUTPUT:
%   NM - Updated NeuroMiner structure with training results stored in analysis.GDdims
%
% USAGE:
%   NM = nm_train(NM, 1);
%   NM = nm_train(NM, 1, inp_struct);

% =========================================================================
% Internal training logic begins below
% =========================================================================
