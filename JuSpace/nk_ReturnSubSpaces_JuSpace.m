function [Y, Thresh] = nk_ReturnSubSpaces_JuSpace(V, Vm, n_samples, n_subjects, Thresh, interpolation, reportfile)
% ===============================================================================
...
%                     fprintf('.')
                    try
                        if iscell(V)
                            d = spm_slice_vol( V{j}(i), M1, Vm.dim(1:2), interpolation );
                        else
                            d = spm_slice_vol( V(i), M1, Vm.dim(1:2), interpolation );
                        end
                    catch
                        if iscell(V)
